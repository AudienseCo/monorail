...
module.exports = function(githubApi, config) {
  ...
  that.getIssueLabels = (repo, issue, cb) => {
    callbackify(githubApi.issues.listLabelsOnIssue)(buildMsg({ issue_number: issue, repo }), (err, res) => handleResponse(err, res, cb));
  };

  that.addIssueLabels = (repo, issue, labels, cb) => {
    const msg = buildMsg({ issue_number: issue, labels: labels, repo });
    callbackify(githubApi.issues.update)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.updateCommitStatus = (options, cb) => {
    const msg = buildMsg(options);
    callbackify(githubApi.repos.createStatus)(msg, (err, res) => handleResponse(err, res, cb));
  };
  ...
  that.getPullRequest = (repo, issue, cb) => {
    ...
    callbackify(githubApi.pulls.get)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.getIssue = (repo, issue, cb) => {
    const msg = buildMsg({ issue_number: issue, repo });
    callbackify(githubApi.issues.get)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.getIssueComments = (repo, issue, cb) => {
    const info = { issue_number: issue, per_page: 100, repo };
    const msg = Object.assign({}, buildMsg(info));
    callbackify(githubApi.issues.listComments)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.createRelease = (info, cb) => {
    const msg = Object.assign({}, buildMsg(info));
    callbackify(githubApi.repos.createRelease)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.compareCommits = (info, cb) => {
    const msg = Object.assign({}, buildMsg(info));
    callbackify(githubApi.repos.compareCommits)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.getBranch = (repo, branch, cb) => {
    const msg = Object.assign({}, buildMsg({ repo, ref: `heads/${branch}` }));
    callbackify(githubApi.git.getRef)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.createBranch = (repo, branch, sha, cb) => {
    const msg = Object.assign({}, buildMsg({ repo, ref: `refs/heads/${branch}`, sha }));
    callbackify(githubApi.git.createRef)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.removeBranch = (repo, branch, cb) => {
    const msg = Object.assign({}, buildMsg({ repo, ref: `heads/${branch}` }));
    callbackify(githubApi.git.deleteRef)(msg, (err, res) => handleResponse(err, res, cb));
  };
  ...
  that.merge = (repo, base, head, cb) => {
    ...
    callbackify(githubApi.repos.merge)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.getContent = (repo, path, ref, cb) => {
    const msg = Object.assign({}, buildMsg({ repo, path, ref }));
    callbackify(githubApi.repos.getContents)(msg, (err, res) => handleResponse(err, res, cb));
  };

  that.getProtectedBranchRequiredStatusChecks = (repo, branch, cb) => {
    const msg = Object.assign({}, buildMsg({ repo, branch }));
    callbackify(githubApi.repos.getProtectedBranchRequiredStatusChecks)(msg, (err, res) => handleResponse(err, res, cb));
  };
